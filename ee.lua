  ...
  function(proxy)
    for i = 1, #OSList do
      if proxy.exists(OSList[i][1]) then
        status(stringsMain, "Uruchamiam z " .. (proxy.getLabel() or proxy.address))
...
status(stringsMain, "Przytrzymaj Alt aby pokazać opcje startu")
...
while uptime() < deadline do
  ...
  if eventData[1] == stringKeyDown and eventData[4] == 56 then
    local utilities = {
      menuElement("Zarządzanie dyskami", function()
        ...
        local function updateFilesystems()
          ...
          for address in componentList(stringsFilesystem) do
            ...
            local label, isReadOnly, filesystemOptions =
              proxy.getLabel() or "Bez nazwy",
              proxy.isReadOnly(),
              {
                menuElement("Ustaw jako bootable", function()
            ...
            if not isReadOnly then
              tableInsert(filesystemOptions, menuElement(stringsChangeLabel, function()
                proxy.setLabel(input(title(2, stringsChangeLabel), "Wpisz nową nazwę: "))
                updateFilesystems()
              end, 1))

              tableInsert(filesystemOptions, menuElement("Format", function()
                status(stringsMain, "Formatuję filesystem " .. address)
